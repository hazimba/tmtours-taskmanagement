...
export function OverdueBarChart({ tasks, users }: OverdueBarProps) {
  const data = useMemo(() => {
    ...
    for (const t of tasks) {
      if (t.status === TaskStatus.COMPLETED || t.status === TaskStatus.CANCELLED) continue;
  ...
  if (data.length === 0)
    return <p className="text-sm text-muted-foreground text-center py-8">No overdue or upcoming tasks. 🎉</p>;

  return (
    <ResponsiveContainer width="100%" height={Math.max(220, data.length * 44)}>
      <BarChart data={data} layout="vertical" margin={{ top: 4, right: 24, left: 8, bottom: 4 }} barCategoryGap="30%">
        ...
        <Bar dataKey="overdue" name="Overdue" fill="#f87171" radius={[0, 4, 4, 0]} stackId="a" />
        <Bar dataKey="soon" name="Due Soon" fill="#fbbf24" radius={[0, 4, 4, 0]} stackId="a" />
      </BarChart>
    </ResponsiveContainer>
  );
}
