...
export function ListPanel({ tasks, currentUserId }: ListPanelProps) {
  ...
  const myOverdue = myTasks.filter(
    (t) => isOverdue(t.due_date) && t.status !== TaskStatus.COMPLETED && t.status !== TaskStatus.CANCELLED
  ).length;
  const mySoon = myTasks.filter(
    (t) => isDueSoon(t.due_date) && !isOverdue(t.due_date) && t.status !== TaskStatus.COMPLETED && t.status !== TaskStatus.CANCELLED
  ).length;
  ...
  const myPriorityCounts = Object.values(TaskPriority).map((p) => ({
    priority: p,
    count: myTasks.filter((t) => t.priority === p && t.status !== TaskStatus.COMPLETED && t.status !== TaskStatus.CANCELLED).length,
  }));
                ...
                <div className="h-1 w-full rounded-full bg-muted overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${pct}%`,
                      backgroundColor:
                        status === TaskStatus.COMPLETED ? "#10b981" :
                        status === TaskStatus.IN_PROGRESS ? "#3b82f6" :
                        status === TaskStatus.REVIEW ? "#f59e0b" :
                        status === TaskStatus.CANCELLED ? "#f87171" : "#94a3b8",
                    }}
                  />
                </div>
              </div>
